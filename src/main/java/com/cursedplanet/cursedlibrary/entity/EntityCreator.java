...
public class EntityCreator {
	...
	public EntityCreator(EntityType type) {
		t = type;
	}
	...
	/**
	 * This method rewrite whole attack damage. but will not register
		...
		try {
			...
			if (set == 0) {
				if (min > 0) {
					...
					MetadataValue metadataValue = new FixedMetadataValue(LibraryPlugin.getInstance(), min);
					e.setMetadata("damage:min", metadataValue);
				}
				if (max > 0) {

					MetadataValue metadataValue = new FixedMetadataValue(LibraryPlugin.getInstance(), max);
					e.setMetadata("damage:max", metadataValue);
				}
			} else {
				MetadataValue metadataValue = new FixedMetadataValue(LibraryPlugin.getInstance(), set);
